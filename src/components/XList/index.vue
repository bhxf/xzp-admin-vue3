    ...
    <div class="flex column col q-gutter-y-xs">
        <div class="tools flex column col-auto q-gutter-y-xs">
            ...
            <div class="q-pl-sm q-pr-sm q-pt-xs">
                <slot name="tools" v-bind="{ticked,selected}"/>
            </div>

            <div class="row no-wrap q-pl-sm q-pr-sm items-center q-gutter-x-xs">
                <q-input class="col" rounded standout dense v-model="search[keyword]" @keyup.enter="loadData"
                         placeholder="请输入关键字">
                    <template #append>
                        <q-icon name="search"/>
                    </template>
...
const loadData = async (params?: any) => {
    if (typeof props.api === 'function') {
        ...
        if (!props.hidePagination) {
            ...
            const pageSize = parseInt(result[props.pagination.pageSize])
            const total = parseInt(result[props.pagination.total])
