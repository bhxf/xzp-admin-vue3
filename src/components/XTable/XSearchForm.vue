<template>
    ...
    >
        <div class="flex column" style="height: 400px;width: 550px;">
            <div class="col-auto q-pa-md">
                <q-input placeholder="请输入关键字" rounded v-model="keyword" standout dense>
                    <template #append>
                        <q-icon name="search"/>
                    </template>
                    <template #after>
                        <q-btn flat round dense color="primary" icon="add" @click="onAdd"/>
                    </template>
                </q-input>
            </div>
            <x-scroll-area>
                <q-list dense padding>
                    <q-item v-for="(item,index) in getList" clickable>
                        <q-item-section>
                            <div class="row no-wrap justify-center items-center q-gutter-x-xs">
                                <q-select
                                    ...
                                    v-model="item.name"
                                    :options="getNames" standout dense
                                    options-dense
                                    emit-value
                                    map-options
                                />
                                ...
                                />
                                <div class="col">
                                    <slot :name="item.name" v-bind="{item,getList}">
                                        <component
                                ...
                                <q-btn v-if="getList.length>1" flat dense round color="red" icon="o_remove_circle" @click="del(index)"/>
                            </div>
                        </q-item-section>
                    </q-item>
                </q-list>
            ...
            <div class="flex justify-center items-center col-auto q-pa-sm">
                <q-btn color="primary" flat rounded label="查询" icon="search" @click="onSearch"/>
                <q-btn color="red" flat rounded label="重置" icon="restart_alt" @click="onReset"/>
            </div>
        </div>
    </q-menu>
</template>
...
import {getComponentsByType, ResultColumn, SearchColumn} from "@/components/XTable/index";
...
const defaultItem: ResultColumn = {name: '', value: null, type: '='}
if (props.searchList.length > 0) {
    defaultItem.name = props.searchList[0].name
    defaultItem.value = props.searchList[0].value
}

const keyword = ref<string>('')
const list = ref<ResultColumn[]>([defaultItem])
...
const getNames = computed(() => {
    return newSearchList.value.map(item => ({label: item.label, value: item.name}))
})
