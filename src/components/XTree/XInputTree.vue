<template>
    <q-field v-model="selected" v-bind="componentsProps" :stack-label="showClear" :loading="loadingState">
        <template #control>
            <template v-if="multiple">
                <q-chip dense v-for="item in selected" :key="item[nodeKey]">
                    {{ formatLabel(item) }}
                </q-chip>
            </template>
            <q-chip v-else-if="selected" dense>{{ formatLabel(selected) }}</q-chip>
        </template>
        <template #append>
            <q-btn v-if="showClear" @click.stop="clearSelected" flat dense round icon="cancel"/>
            <q-icon v-if="required === true" size="xs" name="r_star_border" color="red"/>
        </template>
        <q-popup-proxy>
            <div class="flex column" style="height: 350px;">
                ...
                >
                    ...
                    <template #refresh>
                        <q-btn @click="init" color="primary" flat dense round icon="refresh">
                            <q-tooltip>
                                刷新
                            </q-tooltip>
                        </q-btn>
...
const getSelectedValue = computed(() => {
    return props.multiple ? selected.value.map((item: any) => item[props.nodeKey]) : selected.value?.[props.nodeKey]
})
const showClear = computed(() => {
    return (props.multiple && selected.value.length > 0) || (!props.multiple && selected.value)
})
...
const getSelected = (key: any, list: any[]) => {
    try {
        list.forEach(item => {
            ...
            item.children && getSelected(key, item.children)
        })
    } catch (e) {
...
const clearSelected = () => {
    props.multiple ? selected.value = [] : selected.value = null
}
