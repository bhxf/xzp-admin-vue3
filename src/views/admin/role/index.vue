    ...
    >
        <template v-slot:before>
                ...
                >
                    ...
                    <template #default-header="prop">
                        <div class="flex column justify-start">
                            <div class="text-weight-bold text-caption">{{ prop.node.authorityName }}</div>
                            <div class="text-caption">{{ prop.node.authorityId }}</div>
                        </div>
                    </template>
                </x-tree>
            </div>
            ...
            <x-dialog v-model="loadings['add']">
                <template #title>
                    <div class="text-h6">{{ getTitle }}角色</div>
                </template>
                <x-form ref="formRef" :fields="initRoleForm"/>
                <template #actions>
...
import {createAuthority, deleteAuthority, getAuthorityList, updateAuthority} from "@/api/system/role";
import {actionCondition, actionConst, actionLoading, actionNavTab, actionRef, actionTitle} from "@/tools/action/curd";
import RoleDetail from "@/views/admin/role/detail.vue";
import {useLayoutStore} from "@/store/settings/layout";

const {toTab} = actionNavTab()
const {action,getTitle} = actionTitle('角色')
const {formRef,treeRef,detailRef} = actionRef()
const {loadings} = actionLoading('add','save')
const {search,treeSelected,treeTicked} = actionConst()
